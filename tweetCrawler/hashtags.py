...
try:
    ...
    file = open("Akhilesh_New_Hashtags.json", 'w', encoding='utf-8')  # change the poi
    file_original = open("Akhilesh_New_Hashtags_Original.json", 'w', encoding='utf-8') #change the poi

    # make initial request for most recent tweets (200 is the maximum allowed count)
    new_tweets = api.search(q="#akhileshyadav", count=100, wait_on_rate_limit=True,
                            tweet_mode="extended")  ###change hastag topic
    ...
    for tweet in new_tweets:
        if (True):
            ...
            if (tweetData["in_reply_to_user_id"] != 57948579):
                r10 = {'verified': False}
                # Crawling the HashTags
                hash1 = re.findall(r"#(\w+)", text_en1)
                h = {'hashtags': ' '.join(map(str, hash1))}
                r1 = {'poi_name': "yadavakhilesh"}  ######changePOI
                r2 = {'poi_id': "57948579"}  ######changePOI_ID
                r3 = {"country": "India"}  ######change country
                ...
                b = {'city': 'Lucknow'}  ##########change city

                # Tweet Location : NYC
                d = {'tweet_loc': '26.8467, 80.9462'}  ###########change location
    ...
    while len(new_tweets) > 0:
        ...
        new_tweets = api.search(q="#akhileshyadav", count=100, max_id=oldest, wait_on_rate_limit=True,
                                tweet_mode="extended")  ###change hashtag
